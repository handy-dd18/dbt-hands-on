...
dbtドキュメントサーバはコンテナ内で起動するため、ホストPCのブラウザからアクセスするには以下の2点が必要です：

1. **ポートマッピング** - docker-compose.ymlで設定済み（`8080:8080`）
2. **`--host 0.0.0.0`オプション** - コンテナ外からのアクセスを許可
...
> **Warning**
> `--host 0.0.0.0`を省略すると、サーバはコンテナ内の`127.0.0.1`にのみバインドされ、ホストPCからアクセスできません。
